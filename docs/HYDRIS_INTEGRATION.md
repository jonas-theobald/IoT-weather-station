...
- `User=pi` was hardcoded into the service file, and the script's attempt to fix it matched a dead path (`/home/originaluser`, the original author's own home directory) that never appears in the checked-in file — so the user was silently never corrected. Any install where the actual username isn't literally `pi` (e.g. a custom user set via Raspberry Pi Imager's Advanced Options, as here) failed at boot with "user pi does not exist". Fixed to rewrite `User=` from `$USER`.
