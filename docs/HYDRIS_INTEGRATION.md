...
- **Kernel ext-adv MGMT bug (Pi OS Bookworm, 6.12 rpt kernel, Zero 2 W):** bluetoothd registers advertisements via `Add Extended Advertising Data (0x0055)` and the kernel answers `Invalid Parameters (0x0d)` even for a minimal 3-byte flags payload — every D-Bus advertisement (bluezero, even `bluetoothctl advertise on`) fails, while GATT registration works fine. The legacy MGMT op still works, so the advert is registered out-of-band: `btmgmt add-adv -u <station uuid> -c -g 1`, owned by `ble-advert.service` + `ble-advert.timer` (repo root, script → `/usr/local/sbin/ble-advert-refresh`) — a 60s level-triggered reconciler, because `btmgmt advinfo` can list an instance that is no longer on air (radio churn) and adverts don't resume after a connection drops; both killed the advert silently in the field. The script encodes the desired state — advertising iff radio up AND `bme280` active AND not connected — and the provisioning agent kicks the unit after each EMCON apply so radio changes re-register promptly. Single owner: nothing else may add/remove instance 1 (the old `bme280.service.d/ble-adv-workaround.conf` ExecStartPost drop-in is gone). Diagnose with `btmon` — bluetoothd's own journal only says "Failed to add advertisement".
